...
public class BitmapUtil {
     ...
     * @param result
     */
    public void saveBitmapAsFile(Bitmap bitmap, Observer result) {
        Observable.create(emitter -> {
            ...
            writeBitmap(savePicFl.getPath(), "/pic.jpg", bitmap);
    ...
    private Boolean isHaveSDCard() {
        String SDtate = Environment.getExternalStorageState();
        if (SDtate.equals(Environment.MEDIA_MOUNTED)) {
            return true;
        }
        return false;
    }
     ...
     * @param path 存入路径
     * @param name 名称
     * @param bitmap 存入的Bitmap
     */
    private void writeBitmap(String path, String name, Bitmap bitmap) {
